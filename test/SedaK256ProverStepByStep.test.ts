...
describe('SedaK256Prover: test vectors', () => {
  it('should create same addresses', async () => {
    ...
    expect(wallets[0].address).to.equal(
      '0xCc082f1F022BEA35aC8e1b24F854B36202a3028f'
    );
    expect(wallets[1].address).to.equal(
      '0x79492bD49B1F7B86B23C8c6405Bf1474BEd33CF9'
    );
    expect(wallets[2].address).to.equal(
      '0x1991F8B5b0cCc1B24B0C07884bEC90188f9FC07C'
    );
    expect(wallets[3].address).to.equal(
  ...
  it('should create same sorted Simple Merkle Tree', async () => {
    ...
    expect(tree.root).to.equal(
      '0x7cfabf40bbb7751ee9b7cdc882a3ff7f640290777a7531311df18afca1060bca'
    );
  ...
  it('should create same batch ID', async () => {
    ...
    const initialBatchId = ethers.keccak256(
      ethers.AbiCoder.defaultAbiCoder().encode(
        ...
        [
          ...
          initialBatch.blockHeight,
          initialBatch.validatorRoot,
          initialBatch.resultsRoot,
        ]
      )
    );
    expect(initialBatchId).to.equal(
      '0xcd949028b47d49aea6a35d29f7b344b178ee4cb5b4c6cd24ec1476ef33a40dcd'
    ...
    expect(newBatchId).to.equal(
      '0x912bbbefae54a25cac5a2e15d707a40c7874341a412088f363e71c28f08a7ef6'
    ); // Replace with the actual expected value
  });
  ...
  it('should create same signatures', async () => {
    ...
    expect(validatorWallet.address).to.equal(
      '0xCc082f1F022BEA35aC8e1b24F854B36202a3028f'
    );

    // Sign the new batch ID
    const newBatchIdToSign =
      '0x912bbbefae54a25cac5a2e15d707a40c7874341a412088f363e71c28f08a7ef6';
